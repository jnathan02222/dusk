...
function LinkBox_keyBehavior($this, $symbol) {
    ...
    if (g_Greenfoot_isKeyDown($rt_s(58))) {
        ...
        if (g_Greenfoot_isKeyDown($rt_s(65))){
            navigator.clipboard.readText().then(
            (clipText)=>{
                $symbol = $rt_intern($rt_str(clipText));
...
//Get spotify context
function getSpotifyContext($str){
    var link = $rt_ustr($str);
...
function LinkBox_updateValidity($this) {
    var link = getSpotifyLink(LinkBox_getText($this)).replace(':', '/');
    if(link.includes("track")){
        fetch(`https://api.spotify.com/v1/${link}`, 
        {   method: 'GET', 
            headers: { 
...
function Enemy_run($this) {
    ...
    if ($this.$ySpeed !== 0.0) {
        ...
        if ($intersecting.$size() > 0) {
            ...
            var $border = 60.0;
            $this.$setLocation0($this.$getExactX() - $border, $intersecting.$get0(0).$getExactY() - ($intersecting.$get0(0).$getHeight() / 2 | 0) - 40.0);
            if ($this.$getIntersectingObjects($rt_cls(Building)).$size() > 0){
                $this.$getScrollableWorld().$removeObject0($this);
              
            }else{
                $this.$setLocation0($this.$getExactX() + 2*$border, $intersecting.$get0(0).$getExactY() - ($intersecting.$get0(0).$getHeight() / 2 | 0) - 40.0);
                if ($this.$getIntersectingObjects($rt_cls(Building)).$size() > 0){
                    $this.$getScrollableWorld().$removeObject0($this);
                }else{
                    $this.$setLocation0($this.$getExactX() - $border, $intersecting.$get0(0).$getExactY() - ($intersecting.$get0(0).$getHeight() / 2 | 0) - 40.0);
...
function PlayButton_act($this) {
    ...
    case 0:
        ...
        if (g_Greenfoot_mouseClicked($this) && $this.$ready && !$this.$cooldown) {
            if(spotify_logged_in && $this.$linkBox0.$isValid()){
                $this.$loadingDisplayed = 1;
                //Start music (Playlists and albums too, be resets things, be careful of 500 errors)
                fetch(`https://api.spotify.com/v1/audio-analysis/${getSpotifyLink($this.$linkBox0.$getText()).replace("tracks/", "")}`, 
                    {   method: 'GET', 
                        headers: { 
                            'Content-Type': 'application/json', 
                            'Authorization': `Bearer ${spotify_api_token}` 
                        ...
                        response=>{
                            ...
                            data=>{
                                ...
                                fetch(`https://api.spotify.com/v1/me/player/play?device_id=${spotify_device_id}`, 
                                {   
                                    method: 'PUT', 
                                    body: JSON.stringify({ uris: [getSpotifyContext(LinkBox_getText($this.$linkBox0))] }),
                                    headers: { 
                                        'Content-Type': 'application/json', 
                                        'Authorization': `Bearer ${spotify_api_token}` 
                                    },
...
function Player_run($this) {
    ...
    if (g_Greenfoot_isKeyDown($rt_s(62))) {
        if (!$this.$startedRunning) {
            if(spotify_logged_in && $this.$linkBox1.$isValid()){
    ...
    if ($this.$getExactY() > 1000.0)
        $this.$xSpeed = 0.0;
    if ($this.$getExactX() > $this.$totalDistance + 25)
        $this.$xSpeed = 0.0;
}
...
function City_run($this) {
    ...
    case 1:
        ...
        if(spotify_logged_in && $this.$linkBox2.$isValid()){
            spotify_player.setVolume(0);
        }else{
            SoundManager_stopMusic();
        }
...
function DifficultyButton_act($this) {
    ...
    case 1:
        ...
        SoundManager_playSound($rt_s(17), 50);
        if ($this.$buttonNum == 1)
            $this.$city5.$setDifficulty(0.5);
        else if ($this.$buttonNum == 2)
            $this.$city5.$setDifficulty(2.0);
